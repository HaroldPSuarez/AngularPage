...
export class CarCompareComponent {
  ...
  getWinner(attribute: 'horsepower' | 'year'): Car | null {
    ...
    if (attribute === 'horsepower') {
      return this.selectedCars.reduce((max, car) => (car.horsepower > max.horsepower ? car : max));
    } else {
      return this.selectedCars.reduce((newest, car) => (car.year > newest.year ? car : newest));
    }
  }
